...
public class AbyssalineBlock extends Block {
	...
	public AbyssalineBlock(Settings properties) {
		super(properties);
		this.setDefaultState(this.stateManager.getDefaultState().with(APBlockSettings.CHARGED, false));
	}
...
//	}
	...
	public BlockState getStateForNeighborUpdate(BlockState state, Direction facing, BlockState facingState, WorldAccess worldIn, BlockPos currentPos, BlockPos facingPos) {
		return this.getDefaultState().with(APBlockSettings.CHARGED, checkForNearbyChargedBlocks(worldIn, currentPos));
	}
	
	@Override
	public BlockState getPlacementState(ItemPlacementContext context) {
		return this.getDefaultState().with(APBlockSettings.CHARGED, checkForNearbyChargedBlocks(context.getWorld(), context.getBlockPos()));
	}
	...
	public static boolean checkForNearbyChargedBlocks(WorldAccess world, BlockPos pos) {
		...
		for(Direction sides : Direction.values()) {
			...
			if(state.getBlock() == APBlocks.CHISELED_ABYSSALINE_BRICKS && state.get(APBlockSettings.CHARGED)) {
				powered = true;
				break;
			}
		}
	...
	public static boolean isChargedBlock(BlockState state) {
		return state.getBlock() != APBlocks.CHISELED_ABYSSALINE_BRICKS && state.contains(APBlockSettings.CHARGED) && state.get(APBlockSettings.CHARGED);
	}
